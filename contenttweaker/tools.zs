...
val climax = TraitBuilder.create("climax");
climax.color = 0xffaadd;
climax.localizedName = "traitname";
climax.localizedDescription = "traittxt1§r\n§ftraittxt2";
climax.calcDamage = function(trait, tool, attacker, target, originalDamage, newDamage, isCritical){
 var rate = 0.5 * Math.random() + 0.9;
return newDamage * rate as float;
};
...
dragon3d.localizedName = "融化";
dragon3d.localizedDescription = "雪消冰又释，景和风复暄。§r\n§f你与目标的距离超过一定限度后，距离越远，造成的伤害越低";
...
val evotest = TraitBuilder.create("evotest");
evotest.color = 0xffaadd;
evotest.localizedName = "evotest";
evotest.localizedDescription = "“evotest”§r\n§fevotest";
evotest.addItem(<item:minecraft:iron_bars>);
//进化调试
evotest.onUpdate = function(trait, tool, world, owner, itemSlot, isSelected) {
    //\u5320\u9B42\u8FDB\u5316\u7B49\u7EA7\u4FEE\u6B63
    if (tool.tag.EvolvedTier as int != 3) {
         tool.mutable().updateTag({EvolvedTier: 3});
                }
            ...
            if (materialId == metal) {
                ...
                if (tool.tag.EvolvedTier as int != tier) {
                    if (tier >= 3) {
                        ...
                        tool.mutable().updateTag({EvolvedTier: 3 as int, EvolvedTierExtra: tier as int});
                    }
                }
                break;
            }
...
contemptuous.calcDamage = function(trait, tool, attacker, target, originalDamage, newDamage, isCritical){
    if(attacker.health <( attacker.maxHealth as float * 0.28f)){
        if(target.health >( originalDamage as float * 0.5f)){
         target.attackEntityFrom(IDamageSource.OUT_OF_WORLD() ,  newDamage * 0.5f);    
        return newDamage;
    }else{
       return newDamage; 
    }
...
val powerup = TraitBuilder.create("powerup");
powerup.color = 0xffaadd;
powerup.localizedName = "借力";
powerup.localizedDescription = "四两千斤！§r\n§f进行格挡动作可进入蓄能状态，蓄能期间造成伤害上升并且会获得临时生命";

powerup.calcDamage = function(trait, tool, attacker, target, originalDamage, newDamage, isCritical){
    if(attacker.isPotionActive(<potion:contenttweaker:powerup>)){
        return 1.25f*newDamage;
    }else if(attacker.isPotionActive(<potion:contenttweaker:powerupi>)){
        return 1.5f*newDamage;  
    }else if(attacker.isPotionActive(<potion:contenttweaker:powerupii>)){
        return 1.8f*newDamage;  
    }else{return newDamage;}
   };

powerup.calcCrit = function(thisTrait, tool, attacker, target) {
  if(attacker.isPotionActive(<potion:contenttweaker:powerupii>)){
  attacker.removePotionEffect(<potion:contenttweaker:powerupii>);
        return true;
    }else{return false;}
};
...
val powerupi = TraitBuilder.create("powerupi");
powerupi.color = 0xffaadd;
powerupi.localizedName = "强化借力";
powerupi.localizedDescription = "蓄能爆发！！§r\n§f蓄能持续时间剩余不到四秒时再次格挡可进入强蓄能状态，强蓄能期间伤害与获得的临时生命进一步提升";
powerupi.calcDamage = function(trait, tool, attacker, target, originalDamage, newDamage, isCritical){
   return newDamage;
   };
powerupi.register();

val powerupii = TraitBuilder.create("powerupii");
powerupii.color = 0xffaadd;
powerupii.localizedName = "完备借力";
powerupii.localizedDescription = "我来开眠！！！§r\n§f强蓄能持续时间剩余不到四秒时再次格挡可进入真蓄能状态，真蓄能状态持续时间更短但是伤害增加与获得临时生命更高且攻击必定暴击，真蓄能攻击后会脱离蓄能状态";
powerupii.calcDamage = function(trait, tool, attacker, target, originalDamage, newDamage, isCritical){
   return newDamage;
   };
powerupii.register();

val akanamagic = TraitBuilder.create("akanamagic");
akanamagic.color = 0xffaadd;
akanamagic.localizedName = "霍佩什之责";
akanamagic.localizedDescription = "“手里的镰状剑开始取消”§r\n§f玩家造成伤害后将对一定范围内的生物立即无后摇追加一段等同于造成伤害一定比例的魔法伤害";

akanamagic.calcDamage = function(trait, tool, attacker, target, originalDamage, newDamage, isCritical) {
    var player as IPlayer = attacker;
    var poos as IEntity = target;
    var world as IWorld = player.world;
    var pos as IBlockPos = IBlockPos.create(poos.x, poos.y, poos.z);
    ...
    for entity in entities {
        ...
        if ((entity instanceof IEntityLivingBase) && !(entity instanceof IPlayer)) {
            var living as IEntityLivingBase = entity;
                living.attackEntityFrom(IDamageSource.createEntityDamage("MAGIC", player), newDamage * 0.75f);
        }
    }
    return newDamage;
};
...
val maizosja = TraitBuilder.create("maizosja");
maizosja.color = 0xffaadd;
maizosja.localizedName = "收割";
maizosja.localizedDescription = "“死神降临”§r\n§f玩家造成伤害后将对大范围内的生物追加等同于武器所造成伤害的物理伤害";

maizosja.calcDamage = function(trait, tool, attacker, target, originalDamage, newDamage, isCritical){
    var player as IPlayer =attacker;
    var poos as IEntity =target;
    var world as IWorld  =player.world;
    var pos as IBlockPos = IBlockPos.create(poos.x, poos.y, poos.z);
                var s = pos.getOffset(IFacing.up(), 2).getOffset(IFacing.west(), 7).getOffset(IFacing.north(), 7) as IBlockPos;
                var e = pos.getOffset(IFacing.down(), 2).getOffset(IFacing.east(), 7).getOffset(IFacing.south(), 7) as IBlockPos;
                var entities = world.getEntitiesInArea(s.asPosition3f(), e.asPosition3f()) as IEntity[];
                for entity in entities {
                     var data as IData = world.getBlock(pos).data;
                    if((entity instanceof IEntityLivingBase)&& !(entity instanceof IPlayer)){
                        var living as IEntityLivingBase = entity;
                            living.attackEntityFrom(IDamageSource.createPlayerDamage(player) ,  newDamage * 1.0f);  
                    }
                }
    return newDamage; 
};
...
val superhot = TraitBuilder.create("superhot");
superhot.color = Color.fromHex("9b95c9").getIntColor(); 
superhot.localizedName = game.localize("超高温");
superhot.localizedDescription = game.localize("我来为你指引方向，░░░§r\n§f当玩家生命值不为满时，攻击时提升已损失生命百分比的伤害");
superhot.calcDamage = function(trait, tool, attacker, target, originalDamage, newDamage, isCritical){
    var numhel =(attacker.health / attacker.maxHealth ) as float;
 return newDamage*(1.0f + (1.0f - numhel)) as float;
 };
superhot.register();

val cultivar = TraitBuilder.create("cultivar");
cultivar.color = Color.fromHex("FF8C00").getIntColor(); 
cultivar.localizedName = game.localize("苍院之意");
cultivar.localizedDescription = game.localize("聆听苍白之园的召唤§r\n§f附近的收割柳会协同玩家进行攻击");
cultivar.addItem(<item:palebloom:amber_valve>);
cultivar.onHit = function(trait, tool, attacker, target, damage, isCritical){
  var poos as IEntityLivingBase = target;  //建立IEntity对象，这里指的是攻击目标
    var player as IPlayer = attacker;  //建立IPlayer对象，这里指的是攻击玩家
    var definition = target.definition;
    if (!(target instanceof IEntityLivingBase))return;
    if((!isNull(poos))&&(!isNull(definition))&&(!isNull(definition.id))){  
    var world as IWorld  =player.world;
    var pos as IBlockPos = IBlockPos.create(poos.x, poos.y, poos.z);
                var s = pos.getOffset(IFacing.up(), 32).getOffset(IFacing.west(), 32).getOffset(IFacing.north(), 32) as IBlockPos;
                var e = pos.getOffset(IFacing.down(), 32).getOffset(IFacing.east(), 32).getOffset(IFacing.south(), 32) as IBlockPos;
                var entities = world.getEntitiesInArea(s.asPosition3f(), e.asPosition3f()) as IEntity[];
                for entity in entities {
                    if((entity instanceof IEntityLivingBase)&& !(entity instanceof IPlayer)){
                        var living as IEntity = entity;
                        var livelbs as IEntityLivingBase =entity;
                        if (WorldPaleGarden.isPaleEntity(living.native)){
                        livelbs.revengeTarget = poos;
                         WorldPaleGarden.preformGardenCallEffects(world.native, livelbs.native, poos.native, true); // 播放召唤效果（粒子、音效等）
                        }    
                    }
                }
    }  
...
val bullet1 = TraitBuilder.create("bullet1");
bullet1.color = 0x98F5FF;
bullet1.localizedName = "霜雹弹装填";
bullet1.localizedDescription = "装弹完成§r\n§f你的弹射物将限制敌人行动。若击杀敌人，则将下次潜行射击的弹药强化为霜雹弹";
bullet1.canApplyTogetherTrait = function(thisTrait, otherTrait) {
    return otherTrait != "bullet2" &&  otherTrait != "bullet3"&&  otherTrait != "bullet4"&& otherTrait != "bullet5" ;
};
bullet1.onUpdate = function(trait, tool, world, owner, itemSlot, isSelected) {
    if (owner instanceof IPlayer) {
        var player as IPlayer = owner;
        var item = tool.definition.ores;
        if(item has <ore:Ticbullet>){
             if (!(CotTicTraitLib.hasTicTrait(tool, "moretcon.hailshot1"))) {
                CotTicTraitLib.addTicTrait(tool, "moretcon.hailshot1", 0x98F5FF, 1);
            }
        }
    }
};
...
val bullet2 = TraitBuilder.create("bullet2");
bullet2.color = 0xFF7F24;
bullet2.localizedName = "灼焰弹装填";
bullet2.localizedDescription = "装弹完成§r\n§f你的攻击会赋予敌人一段时间的伤害基于武器一定比例的特殊燃烧状态";
bullet2.canApplyTogetherTrait = function(thisTrait, otherTrait) {
    return otherTrait != "bullet1" &&  otherTrait != "bullet3"&&  otherTrait != "bullet4"&& otherTrait != "bullet5" ;
};
bullet2.onUpdate = function(trait, tool, world, owner, itemSlot, isSelected) {
    if (owner instanceof IPlayer) {
        var player as IPlayer = owner;
        var item = tool.definition.ores;
        if(item has <ore:Ticbullet>){
             if (!(CotTicTraitLib.hasTicTrait(tool, "moretcon.hyperheat"))) {
                CotTicTraitLib.addTicTrait(tool, "moretcon.hyperheat", 0xFF7F24, 1);
            }
        }
    }
};
...
val bullet3 = TraitBuilder.create("bullet3");
bullet3.color = 0xB0C4DE;
bullet3.localizedName = "蓄雷弹装填";
bullet3.localizedDescription = "装弹完成§r\n§f你的攻击会使弹射物积蓄电荷，电荷积累到一定程度后会使弹射物进入感电状态并增强武器的威力，暴击可以加快电荷的积累过程，若过长时间没有攻击则电荷会被清空";
bullet3.canApplyTogetherTrait = function(thisTrait, otherTrait) {
    return otherTrait != "bullet1" &&  otherTrait != "bullet2"&&  otherTrait != "bullet4"&& otherTrait != "bullet5" ;
};
bullet3.onUpdate = function(trait, tool, world, owner, itemSlot, isSelected) {
    if (owner instanceof IPlayer) {
        var player as IPlayer = owner;
        var item = tool.definition.ores;
        if(item has <ore:Ticbullet>){
             if (!(CotTicTraitLib.hasTicTrait(tool, "moretcon.electrified"))) {
                CotTicTraitLib.addTicTrait(tool, "moretcon.electrified", 0xB0C4DE, 1);
            }
        }
    }
};
...
val bullet4 = TraitBuilder.create("bullet4");
bullet4.color = 0x00CD00;
bullet4.localizedName = "叶绿弹装填";
bullet4.localizedDescription = "装弹完成§r\n§f你的弹射物将在障碍物表面弹射并追踪附近目标，弹射次数越多，额外伤害越高";
bullet4.canApplyTogetherTrait = function(thisTrait, otherTrait) {
    return otherTrait != "bullet1" &&  otherTrait != "bullet2"&&  otherTrait != "bullet3"&& otherTrait != "bullet5" ;
};
bullet4.onUpdate = function(trait, tool, world, owner, itemSlot, isSelected) {
    if (owner instanceof IPlayer) {
        var player as IPlayer = owner;
        var item = tool.definition.ores;
        if(item has <ore:Ticbullet>){
             if (!(CotTicTraitLib.hasTicTrait(tool, "moretcon.ricoshot"))) {
                CotTicTraitLib.addTicTrait(tool, "moretcon.ricoshot", 0x00CD00, 1);
            }
        }
    }
};
...
val bullet6 = TraitBuilder.create("bullet6");
bullet6.color = 0xDCDCDC;
bullet6.localizedName = "霰射口套件";
bullet6.localizedDescription = "安装完成§r\n§f你的远程武器会在发射时像霰弹枪一样同时射出数枚削弱过的投掷物";
bullet6.onUpdate = function(trait, tool, world, owner, itemSlot, isSelected) {
    if (owner instanceof IPlayer) {
        var player as IPlayer = owner;
        var item = tool.definition.ores;
             if (!(CotTicTraitLib.hasTicTrait(tool, "moretcon.polyshot"))) {
                CotTicTraitLib.addTicTrait(tool, "moretcon.polyshot", 0xDCDCDC, 1);
            }
        }
...
val bullet7 = TraitBuilder.create("bullet7");
bullet7.color = 0xDCDCDC;
bullet7.localizedName = "释放销套件";
bullet7.localizedDescription = "安装完成§r\n§f你的远程武器长按右键时将能够进行全自动射击，并且射击速度会更快（仅对火枪与十字弩有效）";
bullet7.onUpdate = function(trait, tool, world, owner, itemSlot, isSelected) {
    if (owner instanceof IPlayer) {
        var player as IPlayer = owner;
        var item = tool.definition.ores;
             if (!(CotTicTraitLib.hasTicTrait(tool, "moretcon.autoloading"))) {
                CotTicTraitLib.addTicTrait(tool, "moretcon.autoloading", 0xDCDCDC, 1);
            }
        }
...
val bullet8 = TraitBuilder.create("bullet8");
bullet8.color = 0xDCDCDC;
bullet8.localizedName = "应力槽套件";
bullet8.localizedDescription = "安装完成§r\n§f武器可蓄力时间提高，此时伤害随蓄力时间提高，飞行轨迹永久保持满射出时的相对直线";
bullet8.onUpdate = function(trait, tool, world, owner, itemSlot, isSelected) {
    if (owner instanceof IPlayer) {
        var player as IPlayer = owner;
        var item = tool.definition.ores;
             if (!(CotTicTraitLib.hasTicTrait(tool, "hytic_chocolate_milk"))) {
                CotTicTraitLib.addTicTrait(tool, "hytic_chocolate_milk", 0xDCDCDC, 1);
            }
        }
...
val bullet5 = TraitBuilder.create("bullet5");
bullet5.color = 0x00CED1;
bullet5.localizedName = "魂噬弹装填";
bullet5.localizedDescription = "装弹完成§r\n§f击杀生物后你的弹射物会吸取敌人的灵魂（存储拥有上限），存储的灵魂会增强你弹射物的效能";
bullet5.canApplyTogetherTrait = function(thisTrait, otherTrait) {
    return otherTrait != "bullet1" &&  otherTrait != "bullet2"&&  otherTrait != "bullet3"&& otherTrait != "bullet4" ;
};
bullet5.onUpdate = function(trait, tool, world, owner, itemSlot, isSelected) {
    if (owner instanceof IPlayer) {
        var player as IPlayer = owner;
        var item = tool.definition.ores;
        if(item has <ore:Ticbullet>){
             if (!(CotTicTraitLib.hasTicTrait(tool, "moretcon.soulforged2"))) {
                CotTicTraitLib.addTicTrait(tool, "moretcon.soulforged2", 0x00CED1, 1);
            }
        }
    }
};
